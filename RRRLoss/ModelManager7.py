...
#         self.model = self.model.to(device)
    ...
    def input_grad_vis(self,batch_idx,epoch,cams):
        ...
        sm_raw_np = cams.clone().detach().cpu().numpy()
        cam_img = []
        cmap_inferno = cm.get_cmap('inferno')

        # grid = make_grid(sm_raw[:6].transpose([1,2,0]),nrow=6,normalize=True,value_range=(-0.5,0.5))*254
        # self.writer2.add_figure('camgrid', grid, batch_idx+epoch*batch_idx)
        ...
        for i in range(5):
            # try:
                ...
                img_temp= sm_raw_np[i].transpose([1,2,0]).clip(0,1)

                fig2 = plt.figure(figsize=(8,10))
                fig2.suptitle(comment,fontsize=12)
                prImg = plt.imshow(img_temp,cmap=cmap_inferno)
                # cam_img.append(prImg)
                self.writer2.add_figure(f'im_{i}', fig2, batch_idx+epoch*batch_idx)
    ...
    def exp_vis_multi(self,batch_idx,epoch,cams,feat8, feat16, feat32):
        ...
        ft8 = feat8.clone().detach()
        ft16 = feat16.clone().detach()
        ...
        for i in range(5):
            # try:
                ...
                plotMe_p = viz.visualize_image_attr(ft8[i][0].unsqueeze(dim=0).cpu().numpy().transpose([1,2,0]),
                            original_image=data[i].cpu().numpy().transpose([1,2,0]),
                            method='heat_map',
                            sign='all', plt_fig_axis=None, outlier_perc=2,
                            cmap='inferno', alpha_overlay=0.3, show_colorbar=True,
    ...
    #             plotMe_p[0].suptitle(comment,fontsize=12)
                ...
                plotMe_n = viz.visualize_image_attr(ft8[i][1].unsqueeze(dim=0).cpu().numpy().transpose([1,2,0]),
                            original_image=data[i].cpu().numpy().transpose([1,2,0]),
                            method='heat_map',
                            sign='all', plt_fig_axis=None, outlier_perc=2,
                            cmap='inferno', alpha_overlay=0.3, show_colorbar=True,
...
#         self.state_dict = state_dict
    ...
    def train_explanation_extractGrad(self):
        ...
        optimizer2 = optim.SGD(self.model.get_params()[0],lr = 0.01,momentum = 0.8)
        exp_lr_scheduler = lr_scheduler.MultiStepLR(optimizer, milestones=[70], gamma=0.1) # 10, 50
        state_dict = {'best_f1': 0., 'precision': 0., 'recall': 0., 'accuracy': 0.}
        i = 0
        ##init eval 
...
#                     param.requires_grad = False
        ...
        # grad_cam = torch.mean(grad_mul, dim=1)
            ...
            # print('testgrad size 0',test_grad[0].size())

            # gradients = self.model.get_activations_gradient3()
            # gradients_sum = torch.sum(test_grad[0], dim=[1],keepdim=True)
            # print(gradients.size())
            # gradients_sum = torch.sum(gradients, dim=[1],keepdim=True)

            

            expl_loss = self.loss_gradient(test_grad[0],log_soft_out,explanation,margin)
            ...
            ##########loss three
            # explanation3 = F.interpolate(explanation,size=(28,28),mode='bilinear')
            # # exp = explanation3.unsqueeze(dim=1)
            # gp_expl = torch.cat([explanation3,1-explanation3],dim=1)
            # expl_loss1 = criteria_BCELog(feat8,gp_expl)

            # explanation4 = F.interpolate(explanation,size=(14,14),mode='bilinear')
            # gp_expl = torch.cat([explanation4,1-explanation4],dim=1)
            # expl_loss2 = criteria_BCELog(feat16,gp_expl)

            # explanation5 = F.interpolate(explanation,size=(7,7),mode='bilinear')
            # gp_expl = torch.cat([explanation5,1-explanation5],dim=1)
            # expl_loss3 = criteria_BCELog(feat32,gp_expl)
            # print(lssss)
            # expl_loss = expl_loss1+expl_loss2+expl_loss3
            ...
            if batch_idx% 20 == 0:
                ...
                # print(self.model.get_activations_gradient3().size())
                self.writer2.add_histogram('hist', self.model.get_activations_gradient3(), batch_idx+ epoch*batch_idx)
                self.input_grad_vis(batch_idx,epoch,test_grad[0])
...
#                       self.writer2.add_histogram(name, sums, batch_idx+ args.epochs*batch_idx)
            ...
            ########visualize samples#########
            # if batch_idx% 20 == 0:
            #     self.exp_vis_multi(batch_idx,epoch,gradients_sum,feat8, feat16, feat32)
