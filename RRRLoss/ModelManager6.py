...
#         self.model = self.model.to(device)
    ...
    def exp_vis_sample(self,batch_idx,epoch):
        ...
        for i in range(10):
            ...
            plotMe = viz.visualize_image_attr(upsampled_attrC_vis[i].detach().cpu().numpy().transpose([1,2,0]),
                        original_image=data[i].detach().cpu().numpy().transpose([1,2,0]),
                        method='heat_map',
                        sign='all', plt_fig_axis=None, outlier_perc=2,
                        cmap='inferno', alpha_overlay=0.3, show_colorbar=True,
#                             title=str(lb[7].cpu()),
                        fig_size=(8, 10), use_pyplot=True)
            a,b = torch.std_mean(upsampled_attrC_vis[i],dim=0)
            comment = 'label:{}, max:{:.6f}, min{:.6f}, avg:{:.6f}, std:{:.6f}'.format(target[i].cpu(),
                torch.max(upsampled_attrC_vis[i]).detach().cpu().numpy(),
                torch.min(upsampled_attrC_vis[i]).detach().cpu().numpy(),
                torch.mean(upsampled_attrC_vis[i]).detach().cpu().numpy(),
                torch.std(upsampled_attrC_vis[i]).detach().cpu().numpy())
            plotMe[0].suptitle(comment,fontsize=12)
            # plotMe.suptitle(comment, fontsize=10, fontweight='bold')
            name='{}/item:{}_iter:{}_epoch:{}_gradcam_possitive_label.jpg'.format(args.pic_viz_dir,
                i,batch_idx,epoch)
            name_b='{}/item:{}_iter:{}_epoch:{}_original.jpg'.format(args.pic_viz_dir,
                i,batch_idx,epoch)

            plotMe[0].savefig(name)
            outImg = data[i].squeeze().detach().cpu().numpy().transpose([1,2,0])
            fig2 = plt.figure(figsize=(8,10))
            prImg = plt.imshow(outImg)
            plt.imshow(explanation[i].squeeze().detach().cpu().numpy(),alpha = 0.2)
            fig2.savefig(name_b)
