...
#                     param.requires_grad = False
        ...
        # grad_cam = torch.mean(grad_mul, dim=1)
            ...
            #     s+= soft_out[i,target[i]]
            # s.backward(retain_graph=True)
            # gradients = self.model.get_activations_gradient3()
            # pooled_gradients = torch.mean(gradients, dim=[0,2,3])
            # grad_mul =  pooled_gradients[None,:,None,None] * feat16 
            # grad_cam = torch.mean(grad_mul, dim=1)
            # expl_loss = criteria_BCEP(grad_cam,explanation)
...
#                           print (name, param.grad.data.view(param.grad.data.size()[0],-1).sum(1))
            ...
            ########visualize samples#########
            if batch_idx% 20 == 0:
                self.exp_vis_multi(batch_idx,epoch,upsampled_attrC)
