...
from loss import OhemCELoss,BCE_possitive_loss,Omid_loss,Omid_loss_menual,Stefano_loss,emb_hinge_loss,BCE_LOGIT
...
#                             title=str(lb[7].cpu()),
    ...
    def exp_vis_multi(self,batch_idx,epoch):
        ...
        sm_raw = upsampled_attrC_vis.clone()
        ...
        for i in range(10):
            try:
                ...
                comment = 'label:{}, max:{:.6f}, min{:.6f}, avg:{:.6f}, std:{:.6f}'.format(target[i].cpu(),
                    torch.max(sm_raw[i]).detach().cpu().numpy(),
                    torch.min(sm_raw[i]).detach().cpu().numpy(),
                    torch.mean(sm_raw[i]).detach().cpu().numpy(),
                    torch.std(sm_raw[i]).detach().cpu().numpy())
                ...
                plotMe_p = viz.visualize_image_attr(sm_p_relu[i].detach().cpu().numpy().transpose([1,2,0]),
                            original_image=data[i].detach().cpu().numpy().transpose([1,2,0]),
                            method='heat_map',
                            sign='all', plt_fig_axis=None, outlier_perc=2,
                            cmap='inferno', alpha_overlay=0.3, show_colorbar=True,
    #                             title=str(lb[7].cpu()),
                ...
                plotMe_n = viz.visualize_image_attr(sm_N_relu[i].detach().cpu().numpy().transpose([1,2,0]),
                            original_image=data[i].detach().cpu().numpy().transpose([1,2,0]),
                            method='heat_map',
                            sign='all', plt_fig_axis=None, outlier_perc=2,
                            cmap='inferno', alpha_overlay=0.3, show_colorbar=True,
...
#                     param.requires_grad = False
        ...
        for batch_idx, (data,explanation, target) in enumerate(train_loader):
            ...
            gc_attrC = layer_gc.attribute(data, target=target, relu_attributions=False)
            upsampled_attrC = LayerAttribution.interpolate(gc_attrC, (224, 224))
            expl_loss = criteria_omid(upsampled_attrC,explanation)
            loss_all = expl_loss
            loss_all.backward()
            optimizer2.step()
...
#                       self.writer2.add_histogram(name, sums, batch_idx+ args.epochs*batch_idx)
            ...
            ########visualize samples#########
            # if batch_idx% 20 == 0:
            #     self.exp_vis_multi(batch_idx,epoch)
