        ...
        for epoch in range(args.epoch):
            ...
            for idx,(X,y) in enumerate(self.train_loader):
                ...
                if idx% args.log_interval == 0:
                    ...
                    print(f_final)                
                    # self.writer.add_scalar('normal_loss',lss,idx+ epoch*idx,display_name='normal_loss')
                    # self.writer.add_scalar('precision',precision,idx+ epoch*idx,display_name='precision')
                    # self.writer.add_scalar('recall',recall,idx+ epoch*idx,display_name='recall')
                    # self.writer.add_scalar('fscore',fscore,idx+ epoch*idx,display_name='fscore')

                    #start visualization and add to tensorboard
                    grd_img = self.vis_fixed_inputgrad(model)
                    self.writer.add_image(f'fixed_samples_{idx+ epoch*idx}',grd_img,global_step = idx+ epoch*idx)
        ...
        for epoch in range(args.epoch):
            ...
            for idx,(X,y) in enumerate(self.train_loader):
                ...
                if idx% args.log_interval == 0:
                    ...
                    print(f_final)                
                    self.writer.add_scalar('normal_loss',lss,idx+ epoch*idx)
