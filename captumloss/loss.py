...
class BCE_possitive_loss(nn.Module):
    ...
    def forward(self, sampels, explanations):
        ...
        smpl = F.relu(sampels)
        smpl = F.sigmoid(smpl).view_as(sampels)

        loss = self.criteria(smpl, explanations)
        return torch.mean(loss)
...
class Omid_loss(nn.Module):
    ...
    def forward(self, samples, explanations):
        ...
        Two_BCE_loss = self.criteria(upsampled_attr_positive_relu,explanations) + self.criteria(upsampled_attr_negetive_relu,1-explanations)
