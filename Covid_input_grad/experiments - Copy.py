...
class experiment:
    ...
    def vis_average_grad(self,avg_grad):
      ...
      for i in range(x):
        ...
        ax.scatter(np.linspace(0,y,y),avg_grad_array[i,:]-0.1*i, c ='blue')
      return fig
    ...
    def vis_fixed_IntegratedGradient(self,model,n_step):
        ...
        vis_img = torch.cat(    
            ...
            dim = 0)
        #clamp values
        vis_img = torch.clip(vis_img,min=0,max=1)
        ##seaborn 
...
#                     param.requires_grad = False
        ...
        for batch_idx, (data,explanation, target) in enumerate(train_loader):
            ...
            if batch_idx % args.log_interval == 0:        
                ...
                grd_img = self.vis_fixed_inputgrad(model)
                id_image,fig_image = self.vis_fixed_IntegratedGradient(model,20)
                ...
                self.writer2.add_figure('fixed_samples_IG',fig_image,global_step = batch_idx+ epoch*batch_idx,close =True)
